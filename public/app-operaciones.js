(function () {
  ...
  async function deleteContribution(movement, member) {
    if (!movement) return;
    if (!confirm(`¿Eliminar definitivamente el recibo ${movement.recibo_numero}? El pago desaparecerá del listado y de la base de datos.`)) return;
    try {
      if (client && backendOperativo) { const result = await client.rpc("fn_eliminar_movimiento_aporte", { p_id: movement.id }); if (result.error) throw result.error; }
      state.aportes = state.aportes.filter((row) => row.id !== movement.id); saveLocal();
      closeModal(); renderContributions(); renderReports(); if (member) setTimeout(() => openMemberAccount(member), 30); toast("Pago eliminado definitivamente");
    }
    catch (err) { toast(friendlyError(err, "No se pudo eliminar el pago")); }
  }
