(function () {
  ...
  const conceptLabel = (value) => ({ aporte_mensual: "Aporte mensual", capital_inicial: "Capital inicial", solidaridad: "Fondo de solidaridad", otro: "Otro concepto" }[value] || "Aporte mensual");
  ...
  function memberPicker(name, selectedId = "", required = false) {
    syncCoreData();
    const selected = findMember(selectedId);
    const options = socios.map((m) => `<option value="${esc(memberSearchLabel(m))}"></option>`).join("");
    return `<input ${required ? "required" : ""} name="${esc(name)}_search" data-member-picker="${esc(name)}" list="${esc(name)}_members" autocomplete="off" value="${esc(selected ? memberSearchLabel(selected) : "")}" placeholder="Buscar por nombre, cédula o número…"><datalist id="${esc(name)}_members">${options}</datalist><input type="hidden" name="${esc(name)}" value="${esc(selectedId)}">`;
  }
  function resolveMemberPicker(form, name, required = false) {
    const input = form.querySelector(`[data-member-picker="${name}"]`);
    const hidden = form.elements[name];
    const query = String(input && input.value || "").trim().toLowerCase();
    if (!query) { hidden.value = ""; return !required; }
    const member = socios.find((m) => memberSearchLabel(m).toLowerCase() === query);
    if (!member) { toast("Seleccioná un socio de la lista de resultados."); return false; }
    hidden.value = member.id; return true;
  }
  ...
  async function updateMemberForPayment(member, payment) {
    if (payment.concepto !== "capital_inicial") return;
    if (client) { await window.cimientosData?.refresh(); syncCoreData(); return; }
    const target = Number(member.capital_suscrito) || 3000000;
    const integrated = Math.min(target, (Number(member.capital_integrado) || 0) + (Number(payment.aporte) || 0));
    const patch = { capital_suscrito: target, capital_integrado: integrated };
    Object.assign(member, patch);
    if (client && backendOperativo) {
      const result = await client.from("solicitudes_socios").update(patch).eq("id", member.id);
      if (result.error) throw result.error;
    } else localStorage.setItem("cimientos_local_v1_solicitudes", JSON.stringify(solicitudes));
  }
  ...
  async function loadBaseData() {
    if (client) {
      const [s, l] = await Promise.all([
        window.cimientosData.readAll("solicitudes_socios"),
        window.cimientosData.readAll("pre_registros")
      ]);
      if (!s.error) solicitudes = s.data || [];
      if (!l.error) preregistros = l.data || [];
    }
    if (!client && !solicitudes.length) solicitudes = JSON.parse(localStorage.getItem("cimientos_local_v1_solicitudes") || "[]");
    if (!client && !preregistros.length) preregistros = JSON.parse(localStorage.getItem("cimientos_local_v1_preregistros") || "[]");
    socios = solicitudes.filter((r) => r.estado === "aprobado");
  }
  async function loadOperations() {
    if (!client) { backendOperativo = false; return; }
    const results = await Promise.all(Object.entries(tables).map(async ([name, table]) => [name, await window.cimientosData.readAll(table)]));
  ...
  async function persist(collection, row) {
    row.updated_at = now();
    if (cloudConfigured && (!client || !backendOperativo)) throw Error("Sin conexión con la base institucional. El registro no fue guardado.");
    if (client && backendOperativo) {
  ...
  function openTask(existing) {
    ...
    modal(existing ? "Editar tarea" : "Nueva tarea", `<form id="opTaskForm" class="op-form"><label class="op-wide">Tarea<input required name="titulo" value="${esc(t.titulo)}"></label><label>Socio relacionado${memberPicker("socio_id", t.socio_id)}</label><label>Responsable<input name="responsable" value="${esc(t.responsable)}"></label><label>Vencimiento<input type="date" name="fecha_vencimiento" value="${esc(t.fecha_vencimiento)}"></label><label>Prioridad<select name="prioridad"><option value="normal">Normal</option><option value="alta"${t.prioridad === "alta" ? " selected" : ""}>Alta</option><option value="urgente"${t.prioridad === "urgente" ? " selected" : ""}>Urgente</option></select></label><label>Estado<select name="estado"><option value="pendiente">Pendiente</option><option value="en_curso"${t.estado === "en_curso" ? " selected" : ""}>En curso</option><option value="completada"${t.estado === "completada" ? " selected" : ""}>Completada</option></select></label>${existing ? '<button type="button" class="btn btn-secondary op-danger" id="opDeleteTask">Eliminar tarea</button>' : ""}${formActions("Guardar tarea")}</form>`);
    bindFormCancel();
    if (existing) $("#opDeleteTask").onclick = () => removeRecord("tareas", t, "tarea", renderTasks);
    $("#opTaskForm").onsubmit = async (e) => { e.preventDefault(); if (!resolveMemberPicker(e.target, "socio_id")) return; const v = Object.fromEntries(new FormData(e.target)); delete v.socio_id_search; try { await persist("tareas", Object.assign(t, v)); closeModal(); renderTasks(); renderReports(); toast("Tarea guardada"); } catch (err) { toast(friendlyError(err, "No se pudo guardar la tarea")); } };
  }

  function renderDocuments() {
    const rows = state.documentos;
    $("#opDocuments").innerHTML = backendNotice() + card(rows.length ? `<div class="tablewrap"><table class="table"><thead><tr><th>Socio</th><th>Documento</th><th>Estado</th><th>Vencimiento</th><th>Registrado por</th><th></th></tr></thead><tbody>${rows.map((d) => `<tr><td>${esc(memberName(findMember(d.socio_id)))}</td><td><strong>${esc(d.tipo)}</strong>${d.nombre_archivo ? `<small class="op-block">${esc(d.nombre_archivo)}</small>` : ""}</td><td>${status(d.estado)}</td><td>${fmtDate(d.fecha_vencimiento)}</td><td>${esc(d.registrado_por || "—")}</td><td><button class="linkbtn" data-document-id="${esc(d.id)}">Consultar / editar</button></td></tr>`).join("")}</tbody></table></div>` : empty("Registrá los documentos que integran cada expediente."));
    $("#opDocuments").insertAdjacentHTML("afterbegin", mobileOpsNav("documentos"));
    $$('[data-document-id]').forEach((b) => b.onclick = () => openDocument(state.documentos.find((d) => d.id === b.dataset.documentId)));
  }
  function openDocument(existing) {
    if (!socios.length) return toast("Primero necesitás un socio aprobado para registrar documentos.");
    const d = existing || { id: uuid(), socio_id: "", tipo: "Cédula de identidad", estado: "vigente", fecha_vencimiento: "", observaciones: "", created_at: now() };
    const types = ["Cédula de identidad","Solicitud de admisión","Comprobante de domicilio","Comprobante de ingresos","Resolución del Consejo","Otro"];
    modal(existing ? "Consultar documento" : "Registrar documento", `<form id="opDocumentForm" class="op-form"><label>Socio${memberPicker("socio_id", d.socio_id, true)}</label><label>Tipo<select name="tipo">${types.map(v=>`<option${d.tipo===v?" selected":""}>${v}</option>`).join("")}</select></label><label>Estado<select name="estado"><option value="vigente"${d.estado==="vigente"?" selected":""}>Vigente</option><option value="observado"${d.estado==="observado"?" selected":""}>Observado</option><option value="faltante"${d.estado==="faltante"?" selected":""}>Faltante</option><option value="vencido"${d.estado==="vencido"?" selected":""}>Vencido</option></select></label><label>Vencimiento<input type="date" name="fecha_vencimiento" value="${esc(d.fecha_vencimiento || "")}"></label><label class="op-wide">${existing && d.nombre_archivo ? `Archivo actual: <strong>${esc(d.nombre_archivo)}</strong><small class="op-field-help">Elegí otro archivo solamente si querés reemplazarlo.</small>` : "Archivo"}<input type="file" id="opDocumentFile" accept=".pdf,image/*"></label><label class="op-wide">Observaciones<textarea name="observaciones" rows="3">${esc(d.observaciones || "")}</textarea></label>${existing && d.storage_path ? '<button type="button" class="btn btn-secondary" id="opViewDocument">Abrir archivo</button>' : ""}${existing ? '<button type="button" class="btn btn-secondary op-danger" id="opDeleteDocument">Eliminar documento</button>' : ""}${formActions(existing ? "Guardar cambios" : "Registrar documento")}</form>`);
    bindFormCancel();
    if (existing && d.storage_path) $("#opViewDocument").onclick = () => viewStoredDocument(d);
    if (existing) $("#opDeleteDocument").onclick = () => deleteDocument(d);
    $("#opDocumentForm").onsubmit = async (e) => { e.preventDefault(); if (!resolveMemberPicker(e.target, "socio_id", true)) return; const v = Object.fromEntries(new FormData(e.target)); delete v.socio_id_search; const file = $("#opDocumentFile").files[0]; const oldPath = d.storage_path || ""; const row = Object.assign({}, d, v, { registrado_por: d.registrado_por || currentUser(), nombre_archivo: file ? file.name : d.nombre_archivo || "" }); try { if (file && client && backendOperativo) { const path = `${v.socio_id}/${row.id}-${Date.now()}-${file.name.replace(/[^a-zA-Z0-9._-]/g, "-")}`; const upload = await client.storage.from("expedientes").upload(path, file, { upsert: false }); if (upload.error) throw upload.error; row.storage_path = path; } else if (file && !backendOperativo) { throw Error("El archivo requiere Supabase activo"); } await persist("documentos", row); if (file && oldPath && oldPath !== row.storage_path) await client.storage.from("expedientes").remove([oldPath]); closeModal(); renderDocuments(); toast(existing ? "Documento actualizado" : "Documento registrado"); } catch (err) { toast(friendlyError(err, "No se pudo guardar el documento")); } };
  }

  async function viewStoredDocument(row) {
    try { const result = await client.storage.from("expedientes").createSignedUrl(row.storage_path, 120); if (result.error) throw result.error; window.open(result.data.signedUrl, "_blank", "noopener,noreferrer"); }
    catch (err) { toast(friendlyError(err, "No se pudo abrir el archivo")); }
  }
  async function deleteDocument(row) {
    if (!confirm(`¿Eliminar ${row.nombre_archivo || row.tipo}? Esta acción no se puede deshacer.`)) return;
    try { await removeRecord("documentos", row, "documento", renderDocuments, true); if (row.storage_path && client) { const removed = await client.storage.from("expedientes").remove([row.storage_path]); if (removed.error) console.warn("Archivo huérfano pendiente de limpieza", removed.error); } }
    catch (err) { toast(friendlyError(err, "No se pudo eliminar el documento")); }
  }
  async function removeRecord(collection, row, label, rerender, skipConfirm = false) {
    if (!skipConfirm && !confirm(`¿Eliminar esta ${label}? Esta acción no se puede deshacer.`)) return;
    try { if (client && backendOperativo) { const result = await client.from(tables[collection]).delete().eq("id", row.id); if (result.error) throw result.error; } state[collection] = state[collection].filter((item) => item.id !== row.id); saveLocal(); closeModal(); rerender(); renderReports(); toast(`${label[0].toUpperCase()+label.slice(1)} eliminado correctamente`); }
    catch (err) { toast(friendlyError(err, `No se pudo eliminar la ${label}`)); }
  }
  ...
  function openMemberAccount(member, year = contributionYear) {
    ...
    const byMonth = new Map(movements.filter((m) => !m.concepto || m.concepto === "aporte_mensual").map((m) => [String(m.periodo).slice(5, 7), m]));
    ...
    const capitalTarget = Number(member.capital_suscrito) || 3000000;
    const capitalIntegrated = Number(member.capital_integrado) || 0;
    const capitalPending = Math.max(0, capitalTarget - capitalIntegrated);
    const founderCapital = String(member.tipo_socio || "") === "fundador" ? `<div class="op-account-summary"><div><span>Capital inicial comprometido</span><strong>${fmtGs(capitalTarget)}</strong></div><div><span>Capital integrado</span><strong>${fmtGs(capitalIntegrated)}</strong></div><div><span>Saldo pendiente</span><strong>${fmtGs(capitalPending)}</strong><small>${capitalPending ? "Seguimiento activo" : "Capital inicial completado"}</small></div></div>` : "";
    modal("Cuenta individual", `<div class="op-account-head"><div><span>Socio N.º ${esc(member.numero_socio || "s/n")}</span><h3>${esc(memberName(member))}</h3><small>C.I. ${esc(member.cedula || "—")}</small></div><label>Año<select id="opAccountYear">${years.map((y) => `<option value="${esc(y)}"${y === year ? " selected" : ""}>${esc(y)}</option>`).join("")}</select></label></div>${founderCapital}<div class="op-account-summary"><div><span>Aportes del año</span><strong>${fmtGs(totals.aporte)}</strong></div><div><span>Solidaridad</span><strong>${fmtGs(totals.solidaridad)}</strong></div><div><span>Total registrado</span><strong>${fmtGs(totals.total)}</strong></div></div><div class="op-month-grid">${months.map((label, i) => { const key = String(i + 1).padStart(2, "0"), movement = byMonth.get(key); return `<div class="op-month ${movement ? "paid" : "pending"}"><span>${label}</span><strong>${movement ? "Pagado" : "Pendiente"}</strong>${movement ? `<small>${esc(movement.recibo_numero || "")}</small>` : ""}</div>`; }).join("")}</div>${movements.length ? `<div class="tablewrap op-account-table"><table class="table"><thead><tr><th>Periodo</th><th>Concepto</th><th>Ver comprobante</th><th>Aporte</th><th>Solidaridad</th><th>Total</th><th></th></tr></thead><tbody>${movements.map((m) => `<tr><td>${esc(m.periodo)}</td><td>${esc(conceptLabel(m.concepto))}</td><td>${esc(m.recibo_numero)}</td><td>${fmtGs(m.aporte)}</td><td>${fmtGs(m.solidaridad)}</td><td>${fmtGs((+m.aporte || 0)+(+m.solidaridad || 0)+(+m.otros || 0))}</td><td><button class="linkbtn" data-account-receipt="${esc(m.id)}">Ver comprobante</button> <button class="linkbtn op-danger" data-delete-contribution="${esc(m.id)}" ${m.estado === "anulada" ? "disabled" : ""}>Anular</button></td></tr>`).join("")}</tbody></table></div>` : empty(`No hay pagos registrados en ${year}.`)}`);
    ...
    $$('[data-delete-contribution]').forEach((b) => b.onclick = () => deleteContribution(state.aportes.find((m) => m.id === b.dataset.deleteContribution), member));
  }
  function renderContributions() {
    syncCoreData();
    const totals = contributionTotals();
    const active = state.aportes.filter((m) => m.estado !== "anulada");
    const monthlyTotal = active.filter((m) => !m.concepto || m.concepto === "aporte_mensual").reduce((sum, m) => sum + (+m.aporte || 0), 0);
    const founderCapitalTotal = active.filter((m) => m.concepto === "capital_inicial").reduce((sum, m) => sum + (+m.aporte || 0), 0);
    const years = Array.from(new Set([today().slice(0,4), ...state.aportes.map((m) => String(m.periodo || "").slice(0,4)).filter(Boolean)])).sort().reverse();
    ...
    const memberResults = contributionSearch ? `<div class="op-member-results">${filteredMembers.length ? filteredMembers.map((m) => { const founder = String(m.tipo_socio || "") === "fundador"; const paid = accountMovements(m.id, contributionYear).filter((x) => !x.concepto || x.concepto === "aporte_mensual").length; const pending = Math.max(0, (Number(m.capital_suscrito) || 3000000) - (Number(m.capital_integrado) || 0)); return `<button class="op-member-result" data-account-member="${esc(m.id)}"><span><strong>${esc(memberName(m))}</strong><small>N.º ${esc(m.numero_socio || "s/n")} · C.I. ${esc(m.cedula || "—")}${founder ? " · Socio fundador" : ""}</small></span><span class="badge ${founder ? (pending ? "pendiente" : "activo") : (paid ? "activo" : "pendiente")}">${founder ? (pending ? `Saldo ${fmtGs(pending)}` : "Capital completo") : `${paid} mes(es)`}</span></button>`; }).join("") : empty("No encontramos socios con esos datos.")}</div>` : "";
    $("#opContributions").innerHTML = backendNotice() + `<div class="op-kpis"><div class="card op-kpi"><span>Aportes mensuales</span><strong>${fmtGs(monthlyTotal)}</strong></div><div class="card op-kpi"><span>Capital fundacional registrado</span><strong>${fmtGs(founderCapitalTotal)}</strong></div><div class="card op-kpi"><span>Fondo de solidaridad</span><strong>${fmtGs(totals.solidaridad)}</strong></div><div class="card op-kpi"><span>Movimientos vigentes</span><strong>${active.length}</strong></div></div>` + card(`<div class="op-contribution-toolbar"><label class="op-search-label">Buscar socio<input class="field" id="opContributionSearch" value="${esc(contributionSearch)}" placeholder="Nombre, cédula, número o teléfono…"></label><label>Año<select class="field" id="opContributionYear">${years.map((y) => `<option value="${esc(y)}"${y === contributionYear ? " selected" : ""}>${esc(y)}</option>`).join("")}</select></label></div>${memberResults}${filteredMovements.length ? `<div class="tablewrap"><table class="table"><thead><tr><th>Ver comprobante</th><th>Socio</th><th>Concepto</th><th>Periodo</th><th>Aporte</th><th>Solidaridad</th><th>Total</th><th>Estado</th><th></th></tr></thead><tbody>${filteredMovements.map((m) => `<tr><td>${esc(m.recibo_numero)}</td><td><button class="op-member-link" data-account-member="${esc(m.socio_id)}">${esc(memberName(findMember(m.socio_id)))}</button></td><td>${esc(conceptLabel(m.concepto))}</td><td>${esc(m.periodo)}</td><td>${fmtGs(m.aporte)}</td><td>${fmtGs(m.solidaridad)}</td><td><strong>${fmtGs((+m.aporte || 0) + (+m.solidaridad || 0) + (+m.otros || 0))}</strong></td><td>${status(m.estado)}</td><td><button class="linkbtn" data-print-contribution="${esc(m.id)}" ${m.estado === "anulada" ? "disabled" : ""}>Ver comprobante</button> <button class="linkbtn op-danger" data-delete-contribution="${esc(m.id)}" ${m.estado === "anulada" ? "disabled" : ""}>Anular</button></td></tr>`).join("")}</tbody></table></div>` : empty("No hay movimientos para los filtros seleccionados.")}`);
    ...
    $$('[data-delete-contribution]').forEach((b) => b.onclick = () => deleteContribution(state.aportes.find((m) => m.id === b.dataset.deleteContribution)));
  ...
  async function openContribution() {
    ...
    modal("Registrar pago", `<form id="opContributionForm" class="op-form"><label class="op-wide">Buscar socio<input required id="opContributionMember" list="opContributionMembers" autocomplete="off" placeholder="Nombre, cédula o número de socio…"><datalist id="opContributionMembers">${memberList}</datalist><small class="op-field-help">Escribí cualquier dato y seleccioná al socio correcto.</small></label><label>Concepto<select name="concepto" id="opContributionConcept"><option value="aporte_mensual">Aporte mensual</option><option value="capital_inicial">Cuota de capital inicial</option><option value="solidaridad">Fondo de solidaridad</option><option value="otro">Otro concepto</option></select></label><label>Periodo<input required type="month" name="periodo" value="${month}"></label><label>Fecha de pago<input required type="date" name="fecha_pago" value="${today()}"></label><label>Aporte / capital (Gs.)<input required inputmode="numeric" data-money name="aporte" value="30.000"></label><label>Solidaridad (Gs.)<input required inputmode="numeric" data-money name="solidaridad" value="20.000"></label><label>Otros conceptos (Gs.)<input inputmode="numeric" data-money name="otros" value="0"></label><label>Forma de pago<select name="forma_pago"><option>Transferencia</option><option>Efectivo</option><option>Depósito</option><option>Otro</option></select></label><label>Número de recibo<input required readonly name="recibo_numero" value="${receipt}"><small class="op-field-help">Asignado automáticamente por el sistema.</small></label><label class="op-wide">Observaciones<textarea name="observaciones" rows="3"></textarea></label>${formActions("Confirmar pago")}</form>`);
    bindFormCancel();
    bindMoneyInputs("#opContributionForm");
    $("#opContributionConcept").onchange = (e) => { if (e.target.value === "capital_inicial") { e.target.form.solidaridad.value = "0"; e.target.form.solidaridad.dispatchEvent(new Event("blur")); } };
    $("#opContributionForm").onsubmit = async (e) => { e.preventDefault(); const v = Object.fromEntries(new FormData(e.target)); const memberQuery = $("#opContributionMember").value.trim().toLowerCase(); const socio = socios.find((m) => memberSearchLabel(m).toLowerCase() === memberQuery); if (!socio) return toast("Seleccioná un socio de la lista de resultados."); if (v.concepto === "capital_inicial" && String(socio.tipo_socio || "") !== "fundador") return toast("La cuota de capital inicial corresponde solamente a socios fundadores."); const row = Object.assign(v, { socio_id: socio.id, id: uuid(), estado: "vigente", registrado_por: currentUser(), created_at: now(), aporte: parseNumber(v.aporte), solidaridad: parseNumber(v.solidaridad), otros: parseNumber(v.otros) }); if (row.concepto === "capital_inicial") { const pending = Math.max(0, (Number(socio.capital_suscrito) || 3000000) - (Number(socio.capital_integrado) || 0)); if (pending <= 0) return toast("Este socio ya completó su capital fundacional."); if (row.aporte <= 0 || row.aporte > pending) return toast(`El pago de capital debe ser mayor a cero y no superar el saldo de ${fmtGs(pending)}.`); } try { await persist("aportes", row); await updateMemberForPayment(socio, row); closeModal(); renderContributions(); renderReports(); toast("Pago registrado y comprobante generado"); printContribution(row); } catch (err) { toast(friendlyError(err, "No se pudo registrar el pago")); } };
  }
  async function deleteContribution(movement, member) {
    if (!movement) return;
    if (movement.estado === "anulada") return toast("Este pago ya está anulado.");
    const motivo = prompt(`Motivo de anulación del recibo ${movement.recibo_numero} (mínimo 5 caracteres):`);
    if (!motivo) return;
    if (motivo.trim().length < 5) return toast("Indicá un motivo de al menos 5 caracteres.");
    if (!confirm("Se descontará de los totales, conservando el recibo y su historial. ¿Continuar?")) return;
    try {
      if (cloudConfigured && (!client || !backendOperativo)) throw Error("No hay conexión para confirmar la anulación.");
      if (client) { const result = await client.rpc("fn_anular_aportes", { p_ids: [movement.id], p_motivo: motivo.trim() }); if (result.error) throw result.error; }
      movement.estado = "anulada"; movement.motivo_anulacion = motivo.trim(); movement.anulado_at = new Date().toISOString(); saveLocal();
      closeModal(); renderContributions(); renderReports(); if (member) setTimeout(() => openMemberAccount(member), 30); toast("Pago anulado; historial conservado");
    }
    catch (err) { toast(friendlyError(err, "No se pudo anular el pago")); }
  }
  async function institutionalPrint(title, body, code, share) {
    ...
    const identity = window.cimientosIdentity?.get() || {};
    const logo = identity.documento_logo || new URL("assets/logo-cimientos.png", location.href).href;
    const institution = identity.documento_nombre || "Cooperativa Cimientos Ltda.";
    const subtitle = identity.documento_subtitulo || "Documento institucional";
    const wa = share && share.phone ? `<button class="whatsapp" onclick="open('https://web.whatsapp.com/send?phone=${String(share.phone).replace(/\D/g, "")}&text=${encodeURIComponent(share.message || "")}', '_blank', 'noopener,noreferrer')" title="Abrir el chat; el PDF se adjunta manualmente">Abrir WhatsApp Web</button>` : "";
    w.document.write(`<!doctype html><html lang="es"><head><meta charset="utf-8"><title>${esc(title)}</title><link rel="stylesheet" href="${new URL("fonts.css", location.href).href}"><style>@page{size:A4;margin:12mm 14mm 14mm}*{box-sizing:border-box}body{font-family:"Nunito Sans",sans-serif;color:#222;margin:0;padding:12mm 14mm 14mm}.head{display:flex;justify-content:space-between;align-items:center;border-bottom:2px solid #3c3e3b;padding-bottom:8px;margin-bottom:18px}.head img{width:178px;height:auto}.head div{text-align:right}.head strong{display:block;color:#27452C;letter-spacing:.02em;font-size:12px}.head small{color:#70746f}.meta{display:flex;justify-content:space-between;color:#777;font-size:10px;margin-bottom:18px}.foot{position:fixed;bottom:8mm;left:14mm;right:14mm;border-top:1px solid #ddd;padding-top:6px;font-size:9px;color:#777;display:flex;justify-content:space-between}h1{font-size:23px;margin:0 0 4px}.receipt-intro{display:flex;justify-content:space-between;align-items:flex-end;margin:0 0 18px}.receipt-intro p{margin:4px 0 0;color:#70746f;font-size:11px}.receipt-number{border-radius:999px;background:#eef4e3;color:#27452C;padding:8px 12px;font-size:12px;font-weight:800}table{width:100%;border-collapse:collapse}thead{display:table-header-group}tr{break-inside:avoid}th{text-align:left;color:#6a6f69;font-size:8px;border-bottom:1.5px solid #333;padding:8px 6px}td{font-size:10px;border-bottom:1px solid #e5e6e2;padding:9px 6px}.box{border:1px solid #b9bdb5;border-radius:18px;padding:20px;background:linear-gradient(145deg,#fff,#fafbf8)}.receipt-grid{display:grid;grid-template-columns:1fr 1fr;gap:0 28px}.receipt-item{padding:10px 0;border-bottom:1px solid #e4e6e0}.receipt-item small{display:block;color:#7b8078;font-size:8px;margin-bottom:3px}.receipt-item strong{font-size:12px}.signs{display:grid;grid-template-columns:1fr 1fr;align-items:end;gap:28px;margin-top:65px;text-align:center;font-size:10px;color:#666}.sign{border-top:1px solid #333;padding-top:6px}.total{display:flex;justify-content:space-between;align-items:center;border-top:1px dashed #bbb;margin-top:18px;padding-top:15px;font-size:17px;font-weight:800}.total span:last-child{font-size:22px;color:#27452C}.receipt-note{font-size:9px;line-height:1.55;color:#70746f;margin-top:18px}.doc-actions{position:fixed;right:18px;bottom:18px;display:flex;gap:8px}.doc-actions button{border:0;border-radius:16px;padding:10px 15px;font:700 12px "Nunito Sans";cursor:pointer}.doc-actions .primary{background:#27452C;color:#fff}.doc-actions .secondary{background:#fff;border:1px solid #ddd}.doc-actions .whatsapp{background:#25D366;color:#fff}@media print{body{padding:0}.doc-actions{display:none}.foot{bottom:0;left:0;right:0}}</style></head><body><header class="head"><img src="${logo}" alt="${esc(institution)}"><div><strong>${esc(title)}</strong><small>${esc(institution)} · ${esc(subtitle)}</small></div></header><div class="meta"><span>Emitido: ${new Intl.DateTimeFormat("es-PY", { dateStyle: "long", timeStyle: "short" }).format(new Date())}</span><span>${esc(code || "")}</span></div>${body}<footer class="foot"><span>${esc(institution)} · Documento generado por el sistema</span><span>${esc(code || "")}</span></footer><div class="doc-actions"><small>Guardá el PDF y adjuntalo manualmente en WhatsApp.</small>${wa}<button class="secondary" onclick="close()">Cerrar</button><button class="primary" onclick="print()">Imprimir / Guardar PDF</button></div></body></html>`);
    w.document.close();
  }
  function printContribution(m) {
    ...
    const founderCapital = m.concepto === "capital_inicial";
    const integrated = socio ? Number(socio.capital_integrado) || 0 : 0;
    const target = socio ? Number(socio.capital_suscrito) || 3000000 : 3000000;
    const receiptTitle = founderCapital ? "Recibo de integración de capital fundacional" : "Recibo de aportes y solidaridad";
    const capitalFields = founderCapital ? `<div class="receipt-item"><small>Capital integrado luego del pago</small><strong>${fmtGs(integrated)}</strong></div><div class="receipt-item"><small>Saldo pendiente</small><strong>${fmtGs(Math.max(0, target - integrated))}</strong></div>` : "";
    institutionalPrint("Comprobante de pago", `<div class="receipt-intro"><div><h1>${receiptTitle}</h1><p>Periodo ${esc(m.periodo)} · Pago registrado el ${fmtDate(m.fecha_pago)}</p></div><span class="receipt-number">${esc(m.recibo_numero)}</span></div><div class="box"><div class="receipt-grid"><div class="receipt-item"><small>Recibí de</small><strong>${esc(memberName(socio))}</strong></div><div class="receipt-item"><small>Socio N.º</small><strong>${esc(socio && socio.numero_socio || "—")}</strong></div><div class="receipt-item"><small>Cédula de identidad</small><strong>${esc(socio && socio.cedula || "—")}</strong></div><div class="receipt-item"><small>Forma de pago</small><strong>${esc(m.forma_pago)}</strong></div><div class="receipt-item"><small>${founderCapital ? "Capital integrado" : "Aporte"}</small><strong>${fmtGs(m.aporte)}</strong></div>${founderCapital ? "" : `<div class="receipt-item"><small>Solidaridad</small><strong>${fmtGs(m.solidaridad)}</strong></div>`}${capitalFields}${Number(m.otros) ? `<div class="receipt-item"><small>Otros conceptos</small><strong>${fmtGs(m.otros)}</strong></div>` : ""}</div><div class="total"><span>Total recibido</span><span>${fmtGs(total)}</span></div></div><p class="receipt-note">Este comprobante acredita el pago registrado en el sistema de la Cooperativa Cimientos Ltda. Conservá el número de recibo para cualquier consulta.</p><div class="signs"><div class="sign">Recibí conforme — Socio</div><div class="sign">Firma y sello — Tesorería</div></div>`, m.recibo_numero, { phone, message: `Hola ${memberName(socio)}. Registramos tu pago correspondiente al periodo ${m.periodo}, recibo ${m.recibo_numero}, por ${fmtGs(total)}. Te enviamos el comprobante en PDF.` });
  }
  ...
  function renderReports() {
    ...
    const annulledButton = document.createElement("button"); annulledButton.className = "btn btn-secondary"; annulledButton.textContent = "Recibos anulados (PDF)";
    $("#opReportPdf").after(annulledButton);
    annulledButton.onclick = () => {
      const cancelled = state.aportes.filter(m => m.estado === "anulada" && (reportPeriod === "todos" || m.periodo === reportPeriod));
      institutionalPrint("Recibos anulados", `<h1>Recibos anulados</h1><p>Periodo: ${esc(reportPeriod)}. Estos importes no integran los totales de recaudación.</p><table><thead><tr><th>Ver comprobante</th><th>Socio</th><th>Importe anulado</th><th>Motivo</th><th>Fecha de anulación</th></tr></thead><tbody>${cancelled.map(m => `<tr><td>${esc(m.recibo_numero)}</td><td>${esc(memberName(findMember(m.socio_id)))}</td><td>${fmtGs(Number(m.aporte)+Number(m.solidaridad)+Number(m.otros))}</td><td>${esc(m.motivo_anulacion || "Anulación histórica sin motivo registrado")}</td><td>${fmtDate(m.anulado_at)}</td></tr>`).join("")}</tbody></table>`, `ANULADOS-${today()}`);
    };
  }
  ...
  function exportOperationalReport(movements) {
    const rows = movements.map((m) => { const socio = findMember(m.socio_id); return { Periodo: m.periodo, Fecha: m.fecha_pago, Recibo: m.recibo_numero, "N.º de socio": socio && socio.numero_socio || "", Socio: memberName(socio), Cédula: socio && socio.cedula || "", Concepto: conceptLabel(m.concepto), Aporte: Number(m.aporte) || 0, Solidaridad: Number(m.solidaridad) || 0, "Otros conceptos": Number(m.otros) || 0, Total: (Number(m.aporte) || 0) + (Number(m.solidaridad) || 0) + (Number(m.otros) || 0), "Forma de pago": m.forma_pago, Estado: m.estado } });
  ...
  function exportMembers() {
    const rows = filteredMembers().map((m) => { const founder = (m.tipo_socio || "ordinario") === "fundador"; const subscribed = founder ? Number(m.capital_suscrito) || 3000000 : 0; const integrated = founder ? Number(m.capital_integrado) || 0 : 0; return { "N.º de socio": m.numero_socio || "", "Apellidos y nombres": memberName(m), "Cédula": m.cedula || "", "Tipo de socio": founder ? "Fundador" : "Ordinario", "Fecha de ingreso": String(m.fecha_ingreso || m.fecha_revision || "").slice(0, 10), "Estado": (m.estado_societario || "activo").replaceAll("_"," "), "Capital comprometido": founder ? subscribed : "", "Capital integrado": founder ? integrated : "", "Saldo pendiente": founder ? Math.max(0, subscribed - integrated) : "", "Estado del capital": founder ? (integrated >= subscribed ? "Completado" : "Pendiente") : "", "Celular": m.celular_whatsapp || m.celular || "", "Correo": m.correo_electronico || "", "Ciudad": m.ciudad || "", "Barrio": m.barrio || "" }; });
  ...
  window.cimientosOperations = { print: institutionalPrint, memberAccount: (id) => { const member = findMember(id); if (!member) return null; const target = Number(member.capital_suscrito) || 3000000; const integrated = Number(member.capital_integrado) || 0; return { target, integrated, pending: Math.max(0, target - integrated), movements: state.aportes.filter((m) => m.estado !== "anulada" && String(m.socio_id) === String(id)) }; }, refresh: async () => { await loadOperations(); syncCoreData(); renderTasks(); renderContributions(); renderDatabase(); renderReports(); } };
