(function () {
  ...
  function openMemberAccount(member, year = contributionYear) {
    ...
    $$('[data-annul-contribution]').forEach((b) => b.onclick = () => annulContribution(state.aportes.find((m) => m.id === b.dataset.annulContribution), member));
  }
  function renderContributions() {
    ...
    $$('[data-annul-contribution]').forEach((b) => b.onclick = () => annulContribution(state.aportes.find((m) => m.id === b.dataset.annulContribution)));
  ...
  async function annulContribution(movement, member) {
    if (!movement || movement.estado === "anulada") return;
    const reason = prompt(`Motivo de anulación del recibo ${movement.recibo_numero}:`);
    if (!reason || !reason.trim()) return toast("La anulación requiere un motivo.");
    if (!confirm("El movimiento quedará anulado, conservará su historial y dejará de sumar en los totales. ¿Continuar?")) return;
    const row = Object.assign({}, movement, { estado: "anulada", observaciones: `${movement.observaciones || ""}\n[ANULADO ${new Date().toLocaleString("es-PY")} por ${currentUser()}] ${reason.trim()}`.trim() });
    try {
      await persist("aportes", row);
      const socio = member || findMember(movement.socio_id);
      if (socio && movement.concepto === "capital_inicial") {
        const patch = { capital_integrado: Math.max(0, (Number(socio.capital_integrado) || 0) - (Number(movement.aporte) || 0)) };
        Object.assign(socio, patch);
        if (client && backendOperativo) { const result = await client.from("solicitudes_socios").update(patch).eq("id", socio.id); if (result.error) throw result.error; }
        else localStorage.setItem("cimientos_local_v1_solicitudes", JSON.stringify(solicitudes));
      }
      closeModal(); renderContributions(); renderReports(); if (member) setTimeout(() => openMemberAccount(member), 30); toast("Pago anulado con trazabilidad");
    }
