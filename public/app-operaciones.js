(function () {
  ...
  function institutionalPrint(title, body, code, share) {
    ...
    const logo = identity.documento_logo || identity.logo || new URL("assets/logo-cimientos.png", location.href).href;
  ...
  function exportMembers() {
    const rows = filteredMembers().map((m) => ({ "N.º de socio": m.numero_socio || "", "Apellidos y nombres": memberName(m), "Cédula": m.cedula || "", "Tipo de socio": (m.tipo_socio || "ordinario") === "fundador" ? "Fundador" : "Ordinario", "Fecha de ingreso": String(m.fecha_ingreso || m.fecha_revision || "").slice(0, 10), "Estado": (m.estado_societario || "activo").replaceAll("_"," "), "Celular": m.celular_whatsapp || m.celular || "", "Correo": m.correo_electronico || "", "Ciudad": m.ciudad || "", "Barrio": m.barrio || "" }));
  ...
  window.cimientosOperations = { print: institutionalPrint, refresh: async () => { await loadOperations(); syncCoreData(); renderTasks(); renderContributions(); renderDatabase(); renderReports(); } };
