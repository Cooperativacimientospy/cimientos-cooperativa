(function () {
  ...
  function rolActual() { return perfilActual && perfilActual.activo !== false ? "superadministrador" : (cloudConfigured ? "lectura" : "superadministrador"); }
  ...
  function openRequest(id) {
    ...
    const canDeleteRequest = !supabaseClient || puedeGestionar();
  ...
  function augmentSettings() {
    const names = ["identidad", "funcionarios", "admision", "mensajes", "seguridad", "respaldo"];
  ...
  function renderSetting(name) {
    ...
    if (name === "funcionarios") {
      ...
      p.innerHTML = head("Funcionarios", "Cada persona tiene su propio usuario, nombre o cargo y trazabilidad. Todos los usuarios activos acceden a todas las funciones.") + `${esSuperadmin() ? '<button class="btn btn-primary" id="inviteStaff" style="margin:12px 0 16px">+ Invitar funcionario</button>' : ''}<div class="list" id="staffList"><div class="empty">Cargando…</div></div>`;
      if (esSuperadmin()) $("#inviteStaff").onclick = () => {
        modal("Invitar funcionario", `<form id="inviteStaffForm" class="op-form"><label>Nombre completo<input required name="nombre" autocomplete="name"></label><label>Correo personal o institucional<input required type="email" name="email" autocomplete="email"></label><label>Teléfono<input name="telefono" inputmode="tel"></label><label>Cargo o descripción<input required name="cargo" placeholder="Ej. Tesorería"></label><div class="op-wide"><small>Recibirá un correo seguro para crear su contraseña. Al activar su cuenta tendrá acceso completo al panel.</small></div><div class="op-wide" style="display:flex;justify-content:flex-end;gap:8px"><button type="button" class="btn btn-secondary" id="cancelInvite">Cancelar</button><button class="btn btn-primary">Enviar invitación</button></div></form>`);
        $("#cancelInvite").onclick = closeModal;
        $("#inviteStaffForm").onsubmit = async (e) => { e.preventDefault(); const values = Object.fromEntries(new FormData(e.target)); const submit = e.submitter; submit.disabled = true; try { const { data, error } = await supabaseClient.functions.invoke("administrar-usuarios", { body: { action: "invite", ...values, rol: "admision", redirectTo: `${siteBase()}/panel.html?invite=1` } }); if (error) throw error; if (data && data.error) throw Error(data.error); closeModal(); toast("Invitación enviada"); renderSetting("funcionarios"); } catch (err) { submit.disabled = false; toast(friendlyError(err, "No se pudo enviar la invitación")); } };
      };
    ...
    if (name === "seguridad") {
      if (supabaseClient) {
        p.innerHTML = head("Seguridad", "El acceso al panel lo protege el login de Supabase — ya no hace falta un PIN local.") + `<div class="item"><div class="item-main"><strong>Tu contraseña</strong><small>Cambiala desde tu perfil (ícono de la esquina superior derecha del panel).</small></div></div><div class="item"><div class="item-main"><strong>Acceso de funcionarios</strong><small>Todo usuario activo tiene acceso completo. Podés activar o desactivar su acceso desde Configuraciones → Funcionarios.</small></div></div>`;
        return;
      }
  ...
  function updateProfileUI() {
    ...
    const cargo = perfilActual ? (perfilActual.cargo || "Funcionario") : (config.cargoAdministrador || "Administrador");
    ...
    $("#profileSave").onclick = busyClick($("#profileSave"), async () => {
      ...
      if (cloud) {
        try {
          ...
          const { data, error } = await supabaseClient.rpc("fn_actualizar_perfil_propio", { p_nombre: n, p_telefono: telefono, p_cargo: cargoNuevo || "Funcionario", p_foto_base64: pendingPhoto });
          if (error) throw error;
